...
- `/groups` - a full-width, three-workspace Groups dashboard. Its group workspaces use live-filtered columns for parent groups/applications, recursively nested child groups/roles, and direct members. The member column includes the selected group's full breadcrumb. `user-manager`, `group-manager-fgap`, and realm admins can browse institute-wide groups, inspect all application roles, and audit a user's memberships. `user-manager` can manage memberships but not group structure. `group-manager-fgap` can create, rename, delete, move, and edit attributes on ordinary groups and application-role descendants, while `/AppRoles` and direct `/AppRoles/{application}` administrator groups remain protected.
...
- `/clients` - a registered application-client audit workspace for realm admins, `client-manager`, and `user-manager`. It correlates each non-system Keycloak client with `/AppRoles/{clientId}`, lists the direct client administrators and application-role subgroups, and exposes administrator membership changes only to realm admins and `client-manager`. It deliberately provides no missing-group repair action.
- `/realm-roles` - a realm-admin-only workspace listing holders of the privileged `client-manager`, `user-manager`, and `group-manager-fgap` realm roles. Realm admins can search existing users and grant or revoke only these explicitly allowlisted roles. Every change is written to the admin-console action audit.
...
- `user-manager` already has FGAP `manage` scope on `Users` and `manage-group-membership` on users and `manage-membership`/`manage-members` on `Groups`.
- `user-manager` has read-only `view` permission on the FGAP `Clients` resource plus `query-clients`, solely for `/clients` auditing.
- `group-manager-fgap` receives FGAP group-management permissions through `policy-group-manager-fgap`; `DelegatedAdminGuardFilter` protects `/AppRoles` and direct `/AppRoles/{application}` administrator roots. `delegated-client-admin-base` is already scoped to its own `AppRoles/{clientId}` subtree by the same filter at the HTTP layer, on top of FGAP.
