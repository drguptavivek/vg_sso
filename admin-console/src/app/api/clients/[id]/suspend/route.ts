...
async function setEnabled(req: NextRequest, id: string, enabled: boolean) {
  ...
  try {
    ...
    if (current.clientId === config.clientId) return NextResponse.json({ error: "The Admin Console client cannot be suspended from this route" }, { status: 403 });
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  return setEnabled(req, id, false);
}
