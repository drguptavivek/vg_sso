...
## 10. Self-Service Group Dashboard

Delegated client admins (`delegated-client-admin-base` holders) no longer need to browse the full Keycloak admin console's realm-wide group tree to manage their own subgroups. The standalone app in [`admin-console/`](../admin-console/README.md) (provisioned by [Step 11](Step_11_AdminConsole.md)) exposes a `/groups` dashboard scoped to only the caller's own `AppRoles/{clientId}` subtree(s): create/rename/delete subgroups and manage membership. The app root itself (`AppRoles/{clientId}`) stays protected, matching this document's Section 4.2 and 7 rules.

This is a curated UI, not a new enforcement layer: every mutation still goes through the standard Admin REST API with the caller's own token, so `DelegatedAdminGuardFilter` remains the authoritative check. The app performs its own ownership check first (using the same "direct `AppRoles/{clientId}` membership" derivation as the filter) purely to return a clear error instead of a raw 403.
