...
Delegated client admins (`delegated-client-admin-base` holders) no longer need to browse the full Keycloak admin console's realm-wide group tree to manage their own subgroups. The standalone app in [`admin-console/`](../admin-console/README.md) exposes a `/groups` dashboard scoped to only the caller's own `AppRoles/{clientId}` subtree(s): create/rename/delete application-role groups and manage membership. The app root itself (`AppRoles/{clientId}`) stays protected, matching this document's Section 4.2 and 7 rules.
